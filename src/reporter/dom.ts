...
`;
    outputHTML += `<hr />`;
    outputHTML += `<div class="testnow-title"> ${results.name} ${
      testsOk
        ? `<span class="testnow-passed">PASSED`
        : `<span class="testnow-failed">FAILED`
    } </span > <span class="testnow-date">${results.date}</span></div>\n`;
    outputHTML += `<ul>`;
    if (results.definitionsOk) {
      results.traverse({
        test(result) {
          if (!result.skipped)
            outputHTML += `<li class="testnow-node testnow-test"> <span class="testnow-title">${
              result.name
            }</span> <span class="testnow-result testnow-${
              result.passed ? `passed">PASSED` : `failed">FAILED`
            } </span> <span class="testnow-elapsed"> ${
              result.elapsed
            } ms</span></li>`;
        },
        group(result) {
          if (!result.skipped) {
            outputHTML += `<li class="testnow-node testnow-group"> <span class="testnow-title">${
              result.name
            }</span> <span class="testnow-result testnow-${
              result.passed ? `passed">PASSED` : `failed">FAILED`
            } </span> <span class="testnow-elapsed"> ${
              result.elapsed
            } ms</span><span class="testnow-count">(${result.passedCount}/${
              result.totalCount
            })</span>`;
            outputHTML += `<ul>`;
          }
        ...
        groupEnd(result) {
          if (!result.skipped) outputHTML += "</ul></li>";
        }
      });
    }
